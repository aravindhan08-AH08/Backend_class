...
def get_all_user(dbs:Session=Depends(connect_to_db)):
    users=dbs.query(customers).all()
    return users

@user_router.get("{id}")
def get_user_by_id(id:int,dbs:Session=Depends(connect_to_db)):
    user_num=dbs.query(customers).filter(customers.id==id).first()
    if not user_num:
...
def edit_user(id:int,user_update:UserSchema,dbs:Session=Depends(connect_to_db)):
    user_new=dbs.query(customers).filter(customers.id==id).first()
    if not user_new:
...
def delete_user(id:int,dbs:Session=Depends(connect_to_db)):
    new_cus=dbs.query(customers).filter(customers.id==id).first()
    if not new_cus:
        raise HTTPException(status_code=404,details="User not found")
    dbs.delete(new_cus)
    dbs.commit()
